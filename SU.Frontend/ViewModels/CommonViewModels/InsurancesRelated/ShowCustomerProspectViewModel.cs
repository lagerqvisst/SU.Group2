...
public class ShowCustomerProspectViewModel : ObservableObject
{
    ...
    public List<ProspectStatus> ProspectStatusOptions =>
        Enum.GetValues(typeof(ProspectStatus)).Cast<ProspectStatus>().ToList();
    ...
    private async Task LoadAllProspectsAsync()
    {
        ...
        {
            Prospects.Clear();
            foreach (var prospect in prospectResult.prospects) Prospects.Add(prospect);
        }
        else
        {
    ...
    private async Task LoadSellersAsync()
    {
        ...
        Sellers.Clear();
        foreach (var seller in sellersResult.salesEmployees) Sellers.Add(seller);
    }
